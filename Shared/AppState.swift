...
struct AppState {
  ...
  var highlightedNote: (Note, Color)? = nil
...
extension AppState {
  var mainScreenState: MainScreenState {
    ...
    if let octaveRange = title?.range(of: "8..") {
      title?[octaveRange].foregroundColor = Color.systemGreen
    }
    ...
    return MainScreenState(
      ...
      isRepeatButtonEnabled: isInteractionEnabled && isPracticing && !isModalVisible,
