...
final class KeyboardView: View {
  ...
  func setTints(_ colors: [Color?], for notes: [Note]) {
  ...
  func setTint(_ color: Color?, for notes: [Note]) {
    for note in notes {
      setTint(color, for: note)
    }
  }

  func setTintForAllNotes(_ color: Color?) {
    for i in 0..<octaveViews.count {
      for note in NoteName.all {
  ...
  func setTint(_ tint: Color?, for note: Note) {
  ...
  func setTint(_ color: Color, percent: Double, for note: Note) {
  ...
  private func setUpViewHierarchy() {
    ...
    scrollView.backgroundColor = .keyBed
